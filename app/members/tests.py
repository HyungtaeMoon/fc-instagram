...
class RelationTestCase(TransactionTestCase):
    ...
    def test_unfollow_if_follow_exists(self):
        u1, u2 = self.create_dummy_user(2)

        # u1이 u2를 follow후 unfollow 실행
        u1.follow(u2)
        u1.unfollow(to_user=u2)

        # u1의 following에 u2가 없어야 함
        self.assertNotIn(u2, u1.following)

    def test_unfollow_fail_if_follow_exists(self):
        u1, u2 = self.create_dummy_user(2)

        # 아래 코드는 올바르지 않아야 함 (Exception)
